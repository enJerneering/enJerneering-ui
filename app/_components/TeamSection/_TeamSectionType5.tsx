...
const TeamItem = ({ person }: { person: People }): JSX.Element => (
  <li>
    <img
      className="aspect-[1/1] w-[80px] rounded-full object-cover"
      src={person.imageUrl}
      alt=""
    />
...
const TeamSectionType5: React.FC<TeamSectionProps> = ({ data }) => {
  const { description, people, tagLine } = data;

  return (
    <div className="bg-gray-50">
      <div className="mx-auto max-w-[1440px] px-6 py-20 lg:p-20">
        <div className="flex flex-col gap-4 max-w-[400px] text-left">
        ...
        >
          {people.map((item, index) => (
            <TeamItem person={item} key={index} />
          ))}
        </ul>
      </div>
    </div>
...
export default TeamSectionType5;
